...
(defun gogetdoc-jump (point &optional other-window)
  "Jump to the definition of the expression at POINT."
  (interactive "d")
  (condition-case nil
      (let* ((iod (gogetdoc-iod point))
             (sig (nth 0 iod))
             (pos (nth 1 iod)))
    ...
    (file-error (message "Could not run gogetdoc binary"))))

(defun gogetdoc-describe (point)
  ...
  (condition-case nil
      (let* ((iod (gogetdoc-iod point))
             (sig (car iod))
             (dcl (cddr iod)))
        (if (or (not (string= "iod-return" sig)) (not dcl))
      ...
      (let* ((iod (gogetdef point))
        ...
        (if (not (string= "gogetdef-return" sig))
          ...
          (if (eval-when-compile (fboundp 'xref-push-marker-stack))
              ...
              (xref-push-marker-stack)
            (ring-insert find-tag-marker-ring (point-marker)))
          (godef--find-file-line-column (mapconcat #'identity pos "") other-window)))
...
(defun gogetdef-describe (point)
  ...
  (condition-case nil
      (let* ((iod (gogetdef point))
             (sig (nth 0 iod))
             (dcl (nth 1 iod)))
        (if (or (not (string= "gogetdef-return" sig)) (not dcl))
            (message "No description found for expression at point")
          (message "%s" dcl)))
    (file-error (message "Could not run gogetdef binary"))))
